...
graph TD
    A[المنزل - القوع] -->|نفايات طعام| B(آلة التحلل السريع)
    B -->|سماد عضوي| C(الزراعة المائية)
    C -->|محصول طازج| A
    D[الطاقة الشمسية] -->|كهرباء نظيفة| A
    D -->|كهرباء نظيفة| C
...
graph LR
    A[مياه الأمطار] --> D(خزان المعالجة)
    B[مياه المكيفات] --> D
    C[مياه المغاسل] --> D
    D -->|فلترة ذكية| E(ري الزراعة المائية)
...
graph LR
    A[تطبيق 'تم'] --> B[صاحب المنزل: خصومات/فواتير]
    A --> C[العامل/الخادم: مكافأة مادية]
    D[البلدية/الشركة] -->|إشراف ومتابعة| A
...
graph LR
    A[بيع المحاصيل الفائضة] --> B[أرباح مالية]
    B --> C[شراء أنظمة طاقة شمسية]
    C --> D{الاستقلال الطاقي للبيت}
    style D fill:#fff9c4,stroke:#fbc02d,stroke-width:4px
